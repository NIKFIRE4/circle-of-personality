...
**КОНТУР.КОСТРОВ** — персональный сервис для осознанного управления временем и жизненным балансом. Он объединяет календарь, цели и сферы жизни в одной системе, показывает, куда фактически уходит неделя, и помогает вовремя замечать перекосы между работой, здоровьем, отношениями, развитием, отдыхом и финансами.

Это не просто список задач: каждое действие становится частью общей картины. Пользователь задаёт недельные ориентиры, распределяет события по сферам и видит свой текущий ритм через наглядный персональный контур.

![Экран обзора](docs/screenshots/overview.png)

## Зачем нужен сервис

Когда календарь, цели и привычки находятся в разных приложениях, сложно понять, соответствует ли реальная неделя личным приоритетам. КОНТУР.КОСТРОВ собирает эти данные вместе и превращает их в понятную картину: какие сферы получают достаточно внимания, какие отстают и на чём стоит сфокусироваться дальше.

## Что можно делать

- планировать события и задачи в интерактивном календаре;
- распределять время по собственным сферам жизни и задавать недельные ориентиры;
- видеть общий баланс, прогресс каждой сферы и главный фокус недели;
- создавать измеримые цели, разбивать их на шаги и связывать действия с календарём;
- добавлять задачи обычной фразой — текстом или голосом — с распознаванием даты, времени и подходящей сферы;
- подключать Google Calendar и Apple Calendar по защищённой iCal/ICS-ссылке;
- автоматически классифицировать импортированные события с помощью ИИ;
- анализировать динамику недели и управлять сферами, профилем и интеграциями;
- пользоваться сервисом с компьютера, планшета или телефона.

Регистрация и пользовательские данные защищены серверными сессиями, ownership-проверками, Zod-валидацией, CSRF-проверкой и журналом значимых действий. В проекте также настроены миграции, seed, health endpoint, Docker Compose и автоматические проверки качества.
